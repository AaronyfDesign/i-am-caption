...
- **Author-hosted free proxy (default, live)**: enabled by default when you open the page — no key needed. Requests relay through the author's Deno Deploy proxy ([neat-whale-5101.aaronyfdesign.deno.net](https://neat-whale-5101.aaronyfdesign.deno.net), reachable from mainland China) to an OpenRouter free vision model. The key lives only in the proxy's environment variables and never reaches the browser; the proxy enforces a free-model allowlist and per-IP rate limiting (10 req / 5 min). Trade-off: a compressed copy of your photo passes through the proxy (nothing is stored); use another mode if that matters;
