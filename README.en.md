...
- **Author-hosted free proxy (default once deployed)**: pick "Author free proxy" under Advanced and start immediately — no key needed. Requests relay through the author's Cloudflare Worker to an OpenRouter free vision model. The key lives only in a Worker secret and never reaches the browser; the Worker enforces a free-model allowlist and per-IP rate limiting (10 req / 5 min). Trade-off: a compressed copy of your photo passes through the proxy (nothing is stored); use another mode if that matters. Off by default — see [worker/README](worker/README.md);
- **No-key manual relay (default without proxy)**: click "Generate via DeepSeek web" — the prompt is copied and chat.deepseek.com opens; drop the photo into the chat, paste the prompt, send, then paste DeepSeek's JSON reply back. Free, powered by the DeepSeek web app itself;
