...
- **Author-hosted free proxy (default, live)**: enabled by default when you open the page — no key needed. Requests relay through the author's Deno Deploy proxy ([neat-whale-5101.aaronyfdesign.deno.net](https://neat-whale-5101.aaronyfdesign.deno.net), reachable from mainland China) to Zhipu BigModel's paid multimodal model **GLM-5.3-Flash** (0.8 CNY/M input, 2.8 CNY/M output, ~0.004 CNY per photo). The key lives only in the proxy's environment variables and never reaches the browser; the proxy pins the model, disables thinking mode, and enforces per-IP rate limiting (10 req / 5 min). Trade-off: a compressed copy of your photo passes through the proxy (nothing is stored); use another mode if that matters;
