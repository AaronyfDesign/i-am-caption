...
- **作者免费代理（默认，已上线）**：打开页面即默认开启，无需任何 Key——请求经作者的 Deno Deploy 代理（[neat-whale-5101.aaronyfdesign.deno.net](https://neat-whale-5101.aaronyfdesign.deno.net)，国内可达）转发到 OpenRouter 免费视觉模型。Key 只存在代理环境变量里，浏览器永远拿不到；代理端强制免费模型白名单 + 限速（每 IP 10 次/5 分钟）。代价：照片压缩副本会经代理一跳（不落盘），介意可换其它模式；
